...
for i in np.arange(n):
    ...
    word_data.song_magnitude = all_sentiments.document_sentiment.magnitude
...
# Top 10 words for each album
top10words = word_data.groupby("album").apply(lambda x: applyf(x, nlargest=10))

# Top 10 types for each album
top10types = word_data.groupby("album").apply(lambda x: applyf(x, type="types", nlargest=10))


# Highest/lowest scoring sentences for each album
top10sentences = word_data.groupby("album").apply(lambda x: applyf(x, type="sentences", nlargest=10))
bottom10sentences = word_data.groupby("album").apply(lambda x: applyf(x, type="sentences", nsmallest=10))
...
# Save outputs
top10sentences.to_csv("nl_data/top10sentences.csv")
bottom10sentences.to_csv("nl_data/bottom10sentences.csv")
top10types.to_csv("nl_data/top10types.csv")
top10words.to_csv("nl_data/top10words.csv")
album_sentiment.to_csv("nl_data/album_sentiment.csv")
album_magnitude.to_csv("nl_data/album_magnitude.csv")
word_data.to_csv("nl_data/raw_nl_data.csv") 